...
import { api, TailorPhase, TailorResponse } from "../api";
...
export default function TailorModal({ masterId, masterName, open, onClose, onCreated }: Props) {
  ...
  // Cancel in-flight stream if the modal closes.
  useEffect(() => {
  ...
  function applyPhase(phase: TailorPhase) {
    setSteps((prev) => {
      ...
      switch (phase.name) {
        case "keywords_start":
          ...
          break;
        case "keywords_done":
          markDone("keywords", `${phase.count} keyword${phase.count === 1 ? "" : "s"}`);
          break;
        case "draft_start":
          setActive("draft", tierLabel(phase.tier));
          break;
        case "draft_done":
        ...
        case "compile_start":
          ...
          break;
        case "compile_done":
          if (phase.page_count === 1) {
            markDone("compile", "1 page");
          } else {
            markDone("compile", `${phase.page_count} pages — repair needed`);
          }
          break;
        case "repair_start":
          setActive(
            "repair",
            `Iteration ${phase.iteration} · ${tierLabel(phase.tier)} · was ${phase.page_count} pages`,
          );
          break;
        case "repair_compile_done":
          // Stay active; the next repair_start (or final result) will move on.
          for (const s of next) {
            if (s.id === "repair") {
              s.detail =
                phase.page_count === 1
                  ? `Iteration ${phase.iteration} · 1 page`
                  : `Iteration ${phase.iteration} · ${phase.page_count} pages`;
            }
          }
          break;
      }
    ...
    try {
      await api.tailorToJd(
        ...
        },
        {
          ...
          onError: (data) => {
            if (data.error === "not_one_page" && data.page_count != null) {
              setError(
                `Tailored resume came out at ${data.page_count} pages after ${data.iterations ?? "?"} repair attempts.`,
              );
            } else {
              setError(data.message || "Tailor failed");
            }
          },
        },
        controller.signal,
